    ...
    for i = x1, x2 do
        for j = y1, y2 do
            ...
            if val == 2 then -- SECRET WALL
                doors = {next = doors, x = i, y = j, color = "secret", open = 0}
            end
...
function checkInteraction()
    ...
    -- IF GREEN KEY
    if mget(player.x, player.y) == 239 then player.greenKey = 1 end
    -- IF BLUE KEY
    if mget(player.x, player.y) == 255 then player.blueKey = 1 end
    -- IF RED KEY
    if mget(player.x, player.y) == 223 then player.redKey = 1 end
    -- IF YELLOW KEY
    if mget(player.x, player.y) == 207 then player.yellowKey = 1 end
end
...
function playerCanMove(x, y)
    val = mget(x, y)
    if val == 1 or val == 3 or val == 39 or val == 10 or val == 11 or val == 12 or
        val == 23 or val == 15 or val == 17 or val == 20 or val == 21 or val ==
        22 or val == 37 or val == 18 then return -1 end
...
function drawPlayer()
    ...
    if player.state == 4 then -- JUMP
        -- En Face
        if player.dir == 0 then
            spr(412, (player.x - camera.x) * 24, (player.y - camera.y) * 24 - 2,
                15, 1, 0, 0, 3, 3)
        end
        if player.dir == 1 then
            spr(364, (player.x - camera.x) * 24, (player.y - camera.y) * 24 - 2,
                15, 1, 0, 0, 3, 3)
        end
        if player.dir == 2 then
            spr(412, (player.x - camera.x) * 24, (player.y - camera.y) * 24 - 2,
    ...
    if player.state == 5 then -- INVENTORY WINDOW DRAW
        ...
        -- En Face
        spr(256, 3 * 24, (player.y - camera.y) * 24, 15, 1, 0, 0, 3, 3)
        spr(271, 3 * 24 + 8, (player.y - camera.y) * 24 + 4, 15, 1, 0, 0, 1, 1)
